...
| 2026-09-05 | Batch 5 | ✅ A 组 9 用例全绿(累计 29 tests);实测:IDLE 锁死(0 炮弹)、start/pause/恢复全通、暂停时 /clock 与 /model_states 停发(物理层冻结的铁证)、恢复后时钟从断点延续(47→51)、击毁靶后广播 WIN | 两层暂停落地:逻辑层广播 /game_state(各节点冻结)+ 物理层调 Gazebo /pause_physics(子弹悬停);胜负日志加了去重(状态变化时打一次);遗留小疑点:击毁瞬间广播的 enemy_alive 出现过 1(疑似取到击毁前一拍),batch 6 多敌人场景复检 |
